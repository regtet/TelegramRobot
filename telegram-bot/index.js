...
function extractBranchFromApkMessage(text) {
    ...
    // 格式 A：✅ APK 打包完成 | 66BB2 | app-bet-66bb.apk
    let m = t.match(/^✅\s*APK\s*打包完成\s*\|\s*([^|]+)\s*\|/m);
    if (m && m[1]) {
        return m[1].trim();
    }
    // 格式 B（用户号当前发送）：✅ APK 打包完成 | 分支名\nAPK地址: https://...
...
bot.on('message', (msg) => {
    ...
    // 文档/图片带说明时成功通知在 caption，需与 text 一并检测
    const textAndCaption = [text, caption].filter((s) => String(s || '').trim()).join('\n');
    ...
    console.log(chalk.gray('  文本内容:'), textAndCaption || text);
    ...
    const apkDoneBody = textAndCaption.trim();
    if (
        apkDoneBody &&
        /^✅\s*APK\s*打包完成\s*\|/m.test(apkDoneBody) &&
        /APK地址:\s*\S+/m.test(apkDoneBody)
    ) {
        const doneBranch = extractBranchFromApkMessage(apkDoneBody);
        if (doneBranch) {
            apkTracker.remove(doneBranch);
            console.log(chalk.green('已从等待打包 APK 列表移除分支:'), doneBranch);
        } else {
            console.log(
                chalk.yellow('收到 APK 完成通知但未能解析分支名，未从 apk-pending.json 移除（请检查文案格式）'),
            );
        }
    }
...
function sendSafe(chatId, message) {
    return bot
        ...
        .catch((err) => {
            console.error(chalk.red('Bot sendMessage 失败:'), err && err.message, String(message).slice(0, 120));
        });
}

/** Telegram 单条文本上限 4096，预留余量，超长时拆成多条「打包APK …」 */
const APK_CMD_MAX_LEN = 3800;

function buildApkCommandChunks(uniqueBranches) {
    const chunks = [];
    let batch = [];
    const flush = () => {
        if (batch.length) {
            chunks.push(`打包APK ${batch.join(' ')}`);
            batch = [];
        }
    };
    for (const br of uniqueBranches) {
        const b = (br || '').trim();
        if (!b) continue;
        const next = batch.length ? [...batch, b] : [b];
        const cmd = `打包APK ${next.join(' ')}`;
        if (cmd.length <= APK_CMD_MAX_LEN) {
            batch = next;
        } else {
            flush();
            const one = `打包APK ${b}`;
            if (one.length > APK_CMD_MAX_LEN) {
                chunks.push(one);
            } else {
                batch = [b];
            }
...
function handleApkCommands(rawText, chatId, messageId) {
    ...
    if (cmd === '/apk_start_all') {
        void runApkStartAll(chatId);
        return bot.deleteMessage(chatId, messageId).catch(() => { });
    }
}

// 执行「一键触发所有等待打包 APK」：将所有分支合并为一条或多条「打包APK xxx」发送给用户机器人
async function runApkStartAll(chatId) {
    const all = apkTracker.getAll();
    if (all.length === 0) {
        await sendSafe(chatId, '📭 当前没有等待打包 APK 的分支');
        return;
    }
    ...
    if (uniqueBranches.length === 0) {
        await sendSafe(chatId, '📭 当前没有有效的分支记录');
        return;
    }

    const chunks = buildApkCommandChunks(uniqueBranches);
    console.log(chalk.cyan('一键启动打包 APK，共'), chunks.length, chalk.cyan('条命令，合计分支数'), uniqueBranches.length);
    if (chunks.length > 1) {
        await sendSafe(
            chatId,
            `📣 分支较多，将分 ${chunks.length} 条「打包APK」消息触发用户号批量打包（请保持用户号进程在线）。`,
        );
    }
...
setInterval(() => {
    ...
    if (hour === 4 && minute === 0 && lastCronRunDay !== today) {
        ...
        if (targetChatId) {
            void sendSafe(targetChatId, '⏰ 凌晨 4 点自动触发：正在执行 /apk_start_all …');
            void runApkStartAll(targetChatId);
