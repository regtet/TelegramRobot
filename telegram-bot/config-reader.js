...
/**
 * 读取指定分支的 config.js 文件并提取 packageId
 * @param {string} projectPath - 项目路径
 * @param {string} branchName - 分支名
 * @returns {Promise<{success: boolean, packageId?: number|string, error?: string}>}
 */
    ...
    try {
        ...
        for (const pattern of patterns) {
            ...
            if (match && match[1]) {
                ...
                // 如果是数字字符串，尝试转换为数字
                const numPackageId = parseInt(packageId, 10);
                return {
                    success: true,
                    packageId: isNaN(numPackageId) ? packageId : numPackageId
                };
            }
        }
